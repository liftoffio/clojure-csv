...
(defn- escaped-quote-at-reader-pos?
  ...
  (.mark reader 2)
  (let [escaped-quote (and (== escape-char (.read reader))
...
(defn- parse-csv-line
  ...
  [^Reader csv-reader delimiter quote-char escape-char strict end-of-line]
...
(defn- parse-csv-with-options
  ([csv-reader {:keys [delimiter quote-char escape-char strict end-of-line]}]
   (parse-csv-with-options csv-reader delimiter quote-char escape-char
                           strict end-of-line))
  ([csv-reader delimiter quote-char escape-char strict end-of-line]
   (lazy-seq
     (when (not (== -1 (reader-peek csv-reader)))
       (let [row (parse-csv-line csv-reader delimiter quote-char escape-char
                                 strict end-of-line)]
         (cons row (parse-csv-with-options csv-reader delimiter quote-char escape-char
                                           strict end-of-line)))))))
  ...
  "Takes a CSV as a string or Reader and returns a seq of the parsed CSV rows,
   ...
   Accepts a number of keyword arguments to change the parsing behavior:
        ...
        :quote-char - A character that is used to begin and end a quoted cell.
                      Default value: \\\"
        :escape-char - A character used to escape nested quotes.
                       Default value: \\\"
...
(defn write-csv
  ...
  [table & {:keys [delimiter quote-char escape-char end-of-line force-quote]
            :or {delimiter \, quote-char \" escape-char \"
                 end-of-line "\n" force-quote false}}]
